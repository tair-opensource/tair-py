...
class TestTairTs:
    @pytest.mark.asyncio
    async def test_exts_p_create(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())

        assert await t.exts_p_create(pkey)

    @pytest.mark.asyncio
    async def test_exts_s_create(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())
        skey = "skey_" + str(uuid.uuid4())
        et = 60 * 1000  # 60 seconds

        assert await t.exts_s_create(
            pkey, skey, et, chunk_size=256, uncompressed=True, labels={"sensor_id": 1}
        )

    @pytest.mark.asyncio
    async def test_exts_s_alter(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())
        skey = "skey_" + str(uuid.uuid4())
        et1 = 60 * 1000
        et2 = 120 * 1000
        ...
        assert await t.exts_s_alter(pkey, skey, et2)
        assert await t.exts_s_alter(pkey, skey)

    @pytest.mark.asyncio
    async def test_exts_s_add(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())
        skey = "skey_" + str(uuid.uuid4())

        assert await t.exts_s_add(
            pkey,
            skey,
            ts=1644310456023,
            value=30.5,
    ...
    @pytest.mark.asyncio
    async def test_exts_s_madd(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())

        assert await t.exts_s_madd(
            pkey,
            [
    ...
    @pytest.mark.asyncio
    async def test_exts_s_incrby(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())
        skey = "skey_" + str(uuid.uuid4())

        assert await t.exts_s_add(pkey, skey, 1644310456023, 30.0)
        assert await t.exts_s_incrby(
            pkey,
            skey,
            1644372093031,
            2,
    ...
    @pytest.mark.asyncio
    async def test_exts_s_mincrby(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())
    ...
    @pytest.mark.asyncio
    async def test_exts_s_del(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())
        ...
        assert await t.exts_s_del(pkey, "temperature")

    @pytest.mark.asyncio
    async def test_exts_s_get(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())
        ...
        result = await t.exts_s_get(pkey, "temperature")
        assert result[0] == 1661419880203
        assert float(result[1]) == pytest.approx(30.2)

    @pytest.mark.asyncio
    async def test_exts_s_info(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())
        ...
        assert await t.exts_s_info(pkey, "temperature") == [
            b"totalDataPoints",
            1,
            b"maxDataPoints",
            0,
    ...
    @pytest.mark.asyncio
    async def test_exts_s_queryindex(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())
        ...
        assert await t.exts_s_queryindex(pkey, ["sensor_id=1"]) == [
            b"distance",
            b"pressure",
            b"temperature",
        ]
    ...
    @pytest.mark.asyncio
    async def test_exts_s_range(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())
    ...
    @pytest.mark.asyncio
    async def test_exts_s_range_aggregation(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())
    ...
    @pytest.mark.asyncio
    async def test_exts_s_mrange(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())
    ...
    @pytest.mark.asyncio
    async def test_exts_s_mrange_aggregation(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())
    ...
    @pytest.mark.asyncio
    async def test_exts_p_range(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())
    ...
    @pytest.mark.asyncio
    async def test_exts_p_range_no_pkey_time_bucket(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())

        with pytest.raises(DataError):
            await t.exts_p_range(
                pkey,
                1661419880202,
                1661419880204,
                pkey_aggregation="SUM",
    ...
    @pytest.mark.asyncio
    async def test_exts_s_raw_modify(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())
        skey = "skey_" + str(uuid.uuid4())

        assert await t.exts_s_add(pkey, skey, 1644310456023, 30.0)
        assert await t.exts_s_raw_modify(
            pkey,
            skey,
            1644310456023,
            31.5,
    ...
    @pytest.mark.asyncio
    async def test_exts_s_raw_mmodify(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())
        ...
        assert await t.exts_s_raw_mmodify(
            pkey,
            [
                TairTsSkeyItem("temperature", 1661419880203, 1),
                TairTsSkeyItem("pressure", 1661419880203, 2),
    ...
    @pytest.mark.asyncio
    async def test_exts_s_raw_incrby(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())
        skey = "skey_" + str(uuid.uuid4())

        assert await t.exts_s_add(pkey, skey, 1644310456023, 30.0)
        assert await t.exts_s_raw_incrby(
            pkey,
            skey,
            1644310456023,
            2.0,
    ...
    @pytest.mark.asyncio
    async def test_exts_s_raw_mincrby(self, t: Tair):
        pkey = "pkey_" + str(uuid.uuid4())
        ...
        assert await t.exts_s_raw_mincrby(
            pkey,
            [
                TairTsSkeyItem("temperature", 1661419880203, 1),
                TairTsSkeyItem("pressure", 1661419880203, 2),
    ...
    @pytest.mark.asyncio
    async def test_tair_ts_skey_item_eq(self):
        skey = "skey_" + str(uuid.uuid4())
    ...
    @pytest.mark.asyncio
    async def test_tair_ts_skey_item_ne(self):
        skey = "skey_" + str(uuid.uuid4())
    ...
    @pytest.mark.asyncio
    async def test_aggregation_repr(self):
        assert (
